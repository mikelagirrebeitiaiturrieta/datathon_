...
"""
# images, mask = load_images('data/images', 'data/mask-one-class/')
...
from models import unet, binary_weighted_cross_entropy
...
input_size = (IMG_HEIGHT, IMG_WIDTH, 3) # (default (256,256,3)) (256,256,1) if grayscale
...
if os.path.exists('data/weights'):
    if 'weights.npy' in os.listdir('data/weights/'):
        weights = np.load('data/weights/weights.npy')
    else:
        weights = get_weights(images) # (default [1.0, 1.0, 1.0, 1.0])
...
print('loss loading...')
# loss = weighted_categorical_crossentropy(weights=weights) # (default 'categorical crossentropy')
loss = binary_weighted_cross_entropy(beta=1.0)
...
model = unet(pretrained_weights = pretrained_weights, input_size = input_size, \
         activation=activation, dropout=dropout, loss=loss, optimizer=optimizer)
...
# images_all = images/images.max()*normalization_value
# mask_all = mask/mask.max()*normalization_value

# test_images_all = test_images_all/test_images_original.max()*normalization_value

results = model.fit(images, mask, validation_split=split, epochs=epochs, batch_size=batch_size,\
                      shuffle=True) 
model.save('model.h5')
