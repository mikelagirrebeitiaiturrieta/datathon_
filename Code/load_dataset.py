...
def load_images(TRAIN_PATH='', TEST_PATH='',\
    ...
    for n,id_ in enumerate(train_ids):
        img = cv2.imread(os.path.join('data/images', id_))
        img = resize(img, (256, 256), mode='constant'\
                        , preserve_range=True)
        images.append(img)
        mask_ = cv2.imread(os.path.join('data/mask-one-class/', id_), cv2.IMREAD_GRAYSCALE)
        mask_ = resize(mask_, (256, 256),\
                        preserve_range=True, mode='constant')
        test_images.append(mask_)
